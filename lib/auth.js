...
export async function editGuard(req) {
  ...
  const emailByOwner = {}
  for (const o of owners) emailByOwner[o.name] = (o.email || '').toLowerCase()
  const ownerByCat = {}
  for (const c of categories) ownerByCat[c.label] = c.owner
  return {
    email,
    unrestricted: false,
    canEdit: (categoryLabel) => {
      const ownerName = ownerByCat[categoryLabel]
      const ownerEmail = ownerName ? emailByOwner[ownerName] : ''
