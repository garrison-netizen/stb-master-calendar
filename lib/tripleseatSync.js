...
/**
 * Take every derived Triple Seat row off the calendar.
 *
 * Scoped hard to Origin = TripleSeat, so a row somebody edited by hand (which
 * flips to Manual) is left alone, and no hand-authored entry is ever touched.
 * The bookings themselves are untouched in the Brain — this only removes the
 * calendar's copies.
 */
export async function purgeTripleSeat(env) {
  const { token, entriesDbId } = env
  if (!token) throw new Error('NOTION_TOKEN is not set')
  if (!entriesDbId) throw new Error('NOTION_DB_ID (calendar entries) is not set')

  const pages = await queryAll(token, entriesDbId, 'Calendar Entries DB')
  const summary = { removed: 0, skippedAdopted: 0, errors: [] }

  for (const page of pages) {
    const e = normalizeEntry(page)
    if (e.origin !== ORIGIN) continue
    if (!e.sourceId) { summary.skippedAdopted++; continue }
    try {
      await patchPage(token, e.id, { archived: true })
      summary.removed++
    } catch (err) {
      summary.errors.push(`archive ${e.sourceId}: ${err.message}`)
    }
    await sleep(120)
  }
  return summary
