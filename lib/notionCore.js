...
// fetch() that retries transient Notion failures (429 + 5xx) with backoff,
// honoring Retry-After. Use ONLY for idempotent reads (queries/GETs) — never
// for create/update, where a retry could duplicate a write. Added 2026-06-25
// so a momentary Notion rate-limit under team load doesn't fail a page load.
...
function normalizePage(page) {
  ...
  return {
    ...
    // Which side of the house: 'Brewery' | 'Coffee'. Null on pre-2026-07-31
    // entries, which render untagged rather than being guessed at.
    business: p.Business?.select?.name || null,
    details: txt(p.Details?.rich_text),
    nothingThisWeek: p['Nothing this week']?.checkbox || false,
  }
}

// A plain day, a timed value / window, or an all-day multi-day span (dateEnd on
// a later day — how one entry covers several weeks). Times are wall-clock
// America/Chicago; the offset-free string + time_zone keeps it correct
// regardless of server zone. A span is always all-day: times are ignored.
function buildDate(payload) {
  const day = payload.date
  if (payload.dateEnd && payload.dateEnd > day)
    return { date: { start: day, end: payload.dateEnd } }
...
function buildProperties(payload) {
  ...
  if (payload.business !== undefined)
    props.Business = {
      select: payload.business ? { name: payload.business } : null,
    }
  return props
}
